...
class Quadcopter():
    ...
    def __init__(self, pos, vel, angle, ang_vel, pos_ref, dt):
        ...
        mass =  1.0 # total mass of the vehicle, kg
        gravity = 9.8 # acceleration due to gravity, m/s^2
        num_motors = 4 # number of motors on the vehicle
        kt = 9.9865e-06 # proportionality constant to convert motor rotational speed into thrust (T=kt*omega^2), N/(rpm)^2
        ...
        self.Ixx = 0.01  # mass-moment of inertial about x-axis, kg-m^2
        self.Iyy = 0.01  # mass-moment of inertial about y-axis, kg-m^2
        self.Izz = 0.02 # mass-moment of inertial about z-axis, kg-m^2
        self.A_ref = 0.02 # reference area for drag calcs, m^2 
        self.L = 0.3 # length from body center to prop center, m
        self.kt = kt # proportionality constant to convert motor rotational speed into thrust (T=kt*omega^2), N/(rpm)^2
        self.b_prop = 1.6e-2 # proportionality constant to convert motor speed to torque (torque = b*omega^2), (N*m)/(rpm)^2
        ...
        self.maxT = 16.5*4 #  max thrust from any single motor, N
    ...
    def find_omegadot(self, omega):
        ...
        #print(self.I, self.tau, omega, self.I, omega)
        quit()
        return omega_dot
    ...
    def des2speeds(self,thrust_des, tau_des):
        ...
        #print(0.5*self.L*self.kt*(motor_speeds[0]+motor_speeds[3]-motor_speeds[1]-motor_speeds[2]))
